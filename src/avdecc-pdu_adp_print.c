...
{
    bool r=true;
    r&=avdecc_pdu_print ( buf,offset,len, "sample_rates = [ " );
    
    if ( fmt->audio_sr_44k1 )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "44K1 " );
    }
    
    if ( fmt->audio_sr_48k )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "48K " );
    }
    
    if ( fmt->audio_sr_88k2 )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "88K2 " );
    }
    
    if ( fmt->audio_sr_96k )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "96K " );
    }
    
    if ( fmt->audio_sr_176k4 )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "176K4 " );
    }
    
    if ( fmt->audio_sr_192k )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "192K " );
    }
    
    r&=avdecc_pdu_print ( buf,offset,len, "] audio_packetization: [ " );
    
    if ( fmt->audio_aaf )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "AAF " );
    }
    
    if ( fmt->audio_float )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "FLOAT " );
    }
    
    r&=avdecc_pdu_print ( buf,offset,len, "] channels: [ " );
    
    if ( fmt->audio_mono )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "1 " );
    }
    
    if ( fmt->audio_2_ch )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "2 " );
    }
    
    if ( fmt->audio_3_ch )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "3 " );
    }
    
    if ( fmt->audio_4_ch )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "4 " );
    }
    
    if ( fmt->audio_5_ch )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "5 " );
    }
    
    if ( fmt->audio_6_ch )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "6 " );
    }
    
    if ( fmt->audio_7_ch )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "7 " );
    }
    
    if ( fmt->audio_8_ch )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "8 " );
    }
    
    if ( fmt->audio_10_ch )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "10 " );
    }
    
    if ( fmt->audio_12_ch )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "12 " );
    }
    
    if ( fmt->audio_14_ch )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "14 " );
    }
    
    if ( fmt->audio_16_ch )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "16 " );
    }
    
    if ( fmt->audio_18_ch )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "18 " );
    }
    
    if ( fmt->audio_20_ch )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "20 " );
    }
    
    if ( fmt->audio_22_ch )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "22 " );
    }
    
    if ( fmt->audio_24_ch )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "24 " );
    }
    
    r&=avdecc_pdu_print ( buf,offset,len, "] max_channels=%d", fmt->max_channels );
...
)
{
    ...
    if ( fmt->video_avf )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "AVF " );
    }
    
    if ( fmt->video_61883_4 )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "61883-4 " );
    }
    
    if ( fmt->video_61883_8 )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "61883_8 " );
    }
    
    if ( fmt->video_iidc )
    {
        r&=avdecc_pdu_print ( buf,offset,len, "IIDC " );
    }
...
)
{
    ...
    r&=avdecc_pdu_print_adp_default_audio_format ( buf,offset,len, &adp->default_audio_format );
    r&=avdecc_pdu_print ( buf,offset,len,"\n%-28s", "Default Video Format:" );
    r&=avdecc_pdu_print_adp_default_video_format ( buf,offset,len, &adp->default_video_format );
    r&=avdecc_pdu_print ( buf,offset,len,"\n%-28s", "Association ID:" );
